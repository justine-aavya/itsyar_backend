...
#     formatted_courses = [
#         CourseItem(id=idx + 1, name=track["name"]) 
#         for idx, track in enumerate(raw_tracks[:2]) # Just grab the first two for landing display
#     ]

#     # Map your existing MOCK_EVENTS structure directly to the landing template properties
#     formatted_hackathons = [
#         HackathonItem(
#             id=idx + 1,
#             title=event["name"],
#             date=f"{event['start_date']} - {event['end_date']}",
#             # Dynamically convert registered count int into string display metric
#             registrations=f"{event['registered_count']}K+" if event['registered_count'] > 1 else str(event['registered_count'])
